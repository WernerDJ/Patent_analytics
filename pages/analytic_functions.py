...
class Patent_Analysis:
    ...
    def get_top_non_inventor_applicants(self, top_n=20):
        ...
        OPTIMIZED VERSION - Uses RapidFuzz batch processing to eliminate O(n²) loops
        """
        ...
        for _, row in self.filtered_data.iterrows():
            ...
            # OPTIMIZATION: Use RapidFuzz batch matching instead of nested loop
            if inventor_list:  # Only if there are inventors to check
                for applicant in applicant_list:
                    # Find best match against all inventors in ONE call
                    best_match = process.extractOne(
                        applicant.lower(), 
                        [inv.lower() for inv in inventor_list],
                        scorer=fuzz.ratio
                    )
                    
                    # Check if applicant is an inventor (similarity > 85)
                    is_inventor = best_match and best_match[1] > 85
                ...
                # No inventors, so all applicants are non-inventors
                for applicant in applicant_list:
                    non_inventor_applicants_data.append({
                        'Applicant': applicant,
                        'IPC': ipc_codes
        ...
        # OPTIMIZATION: Use RapidFuzz process.cdist for grouping similar names
        ...
        # Process in batches using RapidFuzz
        for name1 in unique_applicants:
            if name1 in grouped_names:
                continue
            
            # Find ALL similar names in ONE optimized call
            ...
            # Create group with all similar names
            group_key = name1
            for match_name, similarity, _ in matches:
                if match_name not in grouped_names:
                    name_groups[group_key].append(match_name)
                    grouped_names.add(match_name)
    ...
    def plot_applicant_parallel_coordinates(self, top_n=10, year_range=None):
        """
        Create parallel coordinates plot for top non-inventor applicants
        
        OPTIMIZED VERSION - Uses vectorized pandas operations and caches fuzzy matches
        """
        ...
        # OPTIMIZATION: Pre-filter by year range once, and use cheap string filter first
        year_filtered_data = self.filtered_data.copy()
        if year_range:
            year_filtered_data = year_filtered_data[
                year_filtered_data['PubYear'].between(start_year, end_year)
            ]
        
        # Count publications per applicant and year
        for applicant in top_applicants:
            # OPTIMIZATION 1: First do a cheap substring search to reduce candidates
            # Use the first 10 characters of the applicant name
            search_term = applicant[:min(10, len(applicant))]
            
            # Quick filter using pandas string operations (much faster than fuzzy on all rows)
            candidate_rows = year_filtered_data[
                year_filtered_data['Applicants'].str.contains(search_term, case=False, na=False, regex=False)
            ]
            
            # OPTIMIZATION 2: Only do fuzzy matching on the filtered candidates (not all rows)
            if len(candidate_rows) > 0:
                ...
                for idx, row in candidate_rows.iterrows():
                    applicants_str = str(row['Applicants']).lower()
                    # Quick check: if applicant name is substring, it's likely a match
                    if applicant.lower() in applicants_str:
                        matching_indices.append(idx)
                    # Otherwise use fuzzy matching
                    elif fuzz.partial_ratio(applicants_str, applicant.lower()) >= 75:
                        matching_indices.append(idx)
                
                if matching_indices:
                    applicant_patents = year_filtered_data.loc[matching_indices]
        ...
        # (Rest of the plotting code remains the same - add axes, labels, etc.)
        host.set_xlim(0, len(all_years) - 1)
        host.set_ylim(0, max([max(applicant_yearly_counts.loc[app].values) for app in top_applicants]) * 1.1)
        host.set_xticks(range(len(all_years)))
        host.set_xticklabels(all_years, fontsize=10, rotation=45)
        host.set_title(f'Top {top_n} Non-Inventor Applicants by Publication Count', fontsize=16, pad=20)
        host.set_xlabel('Publication Year', fontsize=12)
        host.set_ylabel('Number of Publications', fontsize=12)
    ...
    def build_network_graph(self):
        ...
        OPTIMIZED VERSION - Pre-computes inventor sets and uses smart filtering
        """
        ...
        # OPTIMIZATION 1: Pre-compute inventor sets ONCE instead of millions of times
        print("Pre-computing inventor sets...")
        self.network_filtered_data['inventor_set'] = self.network_filtered_data['Inventors'].apply(
            lambda x: set([inv.strip() for inv in str(x).split('; ') if inv.strip()])
        )
        
        # OPTIMIZATION 2: Build an index of which applicants share which inventors
        print("Building inventor-to-applicant index...")
        from collections import defaultdict
        inventor_to_applicants = defaultdict(set)
        
        for idx, row in self.network_filtered_data.iterrows():
            applicant = row['Applicant_Lower']
            for inventor in row['inventor_set']:
                inventor_to_applicants[inventor].add((applicant, idx))
        
        # OPTIMIZATION 3: Only compare applicants who share at least one inventor
        print("Building network graph with smart filtering...")
        G = nx.Graph()
        compared_pairs = set()  # Track which pairs we've already compared
        
        for inventor, applicant_data in inventor_to_applicants.items():
            applicants_list = list(applicant_data)
            
            if len(applicants_list) < 2:
                continue  # Skip inventors who only worked for one applicant
            
            # Only compare applicants who share this inventor
            for i in range(len(applicants_list)):
                for j in range(i + 1, len(applicants_list)):
                    app1, idx1 = applicants_list[i]
                    app2, idx2 = applicants_list[j]
                    
                    # Create a unique pair identifier (sorted to avoid duplicates)
                    pair_key = tuple(sorted([app1, app2]))
                    
                    if pair_key in compared_pairs:
                        continue  # Already compared this pair
                    
                    compared_pairs.add(pair_key)
                    
                    # Get pre-computed inventor sets
                    inventors1 = self.network_filtered_data.iloc[idx1]['inventor_set']
                    inventors2 = self.network_filtered_data.iloc[idx2]['inventor_set']
                    
                    # Find shared inventors
                    shared_inventors = inventors1 & inventors2
                    weight = len(shared_inventors)
                    
                    if weight > 0:
                        if G.has_edge(app1, app2):
                            # Edge already exists, update it
                            G[app1][app2]['weight'] = weight
                            G[app1][app2]['shared_inventors'] = shared_inventors
                        else:
                            # Create new edge
                            G.add_edge(
                                app1,
        ...
        print(f"Compared only {len(compared_pairs)} pairs instead of {len(self.network_filtered_data) * (len(self.network_filtered_data) - 1) // 2}")
    ...
    def _get_openai_key(self):
        """Fetch OpenAI API key from Django settings"""
        import os
        # First try environment variable
        key = os.environ.get("OPENAI_API_KEY")
        if key:
            return key
        # Fall back to Django settings (where AWS Secrets Manager puts it)
        try:
            from django.conf import settings
            return settings.OPENAI_API_KEY
        except Exception as e:
            print(f"Failed to get OpenAI key from Django settings: {e}")
            return None
    ...
    def _translate_labels(self, labels_dict):
        ...
        try:
            client = openai.OpenAI(api_key=self._get_openai_key())
